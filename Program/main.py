...
class main():
    def __init__(self,username):
        ...
        CalFrame.place(height=200,width=300,rely=0,relx=.786)
        t=dt.date.today()
        ...
        self.noten=Label(self.root,text="",bg='#6B8282',fg='white',font=("",12,'bold'),relief=RAISED,height=1)
        ...
        Label(self.root,bg="#b6c71c",height=22,width=6).place(x=1103,y=469)
    ...
    def CalcTime(self):
        endsecs=round(time.time()) 
        tot=endsecs-self.startsecs 
        endtime=dt.datetime.now().strftime('%H:%M:%S')
        date=f"{dt.datetime.now():%a %d %b %Y}" 
    ...
    def OpenNote(self):
        ...
        try:
            self.noten['text']=(self.ON.name)
            opcontent=open(self.ON,'r')
            rtxt=opcontent.read()
        ...
        UN=self.UN
        cwd1=pathlib.Path(cwd+'/dat/usr/'+UN+'/Notes')
        os.chdir(cwd1)
    ...
    def SaveNote(self):
        opcontent=open(self.ON,'w')
        opcontent.write(self.windtext.get('1.0','end'))
    ...
    def Count(self):
        self.cfrmsave['text']="Saved!"
        self.cfrmsave['bg']="white"
        time.sleep(3)
        self.cfrmsave['text']=""
        self.cfrmsave['bg']="#6B8282"

    def SessionClock(self):
        secs=int(time.time()-self.startsecs)
    ...
    class login():
        def __init__(self):
            ...
            Label(self.root,bg="#b6c71c",height=22,width=1).place(x=320,y=0)
